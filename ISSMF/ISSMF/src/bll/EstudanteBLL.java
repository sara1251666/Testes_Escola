...
public class EstudanteBLL {
     ...
     *
     * Validações:
     *  1. NIF duplicado → erro
     *  2. Curso existe e está ativo → erro se não
     *  3. Gera numMec, email institucional, password aleatória
     *  4. Grava estudante + credenciais
     *  5. Envia email (assíncrono, sem mostrar password na consola)
     */
    public OperationResult<Estudante> registar(String nome, String nif, String emailPessoal,
        ...
        if (!curso.estaAtivo()) {
            return OperationResult.error("O curso '" + siglaCurso + "' está inativo.");
        }

        // 3. Gerar numMec e email institucional
        int    numMec     = estudanteDAL.gerarProximoNumMec(anoAtual);
        String emailInst  = numMec + "@issmf.ipp.pt";
        // Se o estudante forneceu um email pessoal, usamos esse; caso contrário, o institucional
        ...
        // Gerar propina inicial = valor do curso
        ...
        if (!authDAL.criarCredencial(emailFinal, passHash, "ESTUDANTE")) {
        ...
        return OperationResult.success(
                "Estudante registado com sucesso! Nº " + numMec
                        + ". Credenciais enviadas para " + emailFinal + ".", estudante);
    }
