 ...
 *
 * O campo 'ativo' (col 10) é opcional para retrocompatibilidade;
 * se ausente assume-se true.
 *
 * LAZY LOADING: todas as pesquisas lêem linha a linha e abortam ao primeiro match.
 * As listas completas (listarDevedores, listarTodos) são excepção justificada pelo requisito.
 */
public class EstudanteDAL {
    ...
    public Estudante obterPorId(int numMec) {
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                if (d.length >= MIN_COLS && DALUtil.parseInt(d[C_NUM_MEC]) == numMec)
                    return mapear(d);  // << abortamos aqui
            }
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "obterPorId", e); }
        return null;
    }
    ...
    public Estudante obterPorEmail(String email) {
        ...
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "obterPorEmail", e); }
        return null;
    }
    ...
    public boolean existeNif(String nif) {
        ...
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "existeNif", e); }
        return false;
    }
    ...
    public boolean existeEmail(String email) {
        ...
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "existeEmail", e); }
        return false;
    }
    ...
    public int contarPorCurso(String siglaCurso) {
        ...
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "contarPorCurso", e); }
        return n;
    }

    /** Conta total de estudantes (exclui o cabeçalho). */
    public int contarTotal() {
        ...
        try (BufferedReader br = abrir()) {
            ...
            String linha;
            while ((linha = br.readLine()) != null) {
                if (!linha.isBlank() && !linha.startsWith("numMec")) n++;
            }
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "contarTotal", e); }
        return n;
    }
    ...
    public List<Estudante> listarDevedores() {
        ...
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                String[] d = linha.split(DELIMITER, -1);
                if (d.length >= MIN_COLS && DALUtil.parseDouble(d[C_SALDO]) > 0)
                    lista.add(mapear(d));
            }
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "listarDevedores", e); }
        return lista;
    }
        ...
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                if (d.length >= MIN_COLS
                        ...
                        && DALUtil.parseInt(d[C_ANO_CURR]) == anoCurricular)
                    lista.add(mapear(d));
            }
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "listarPorCursoEAno", e); }
        return lista;
    }
    ...
    public boolean atualizar(Estudante e) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(orig));
            ...
            while ((linha = br.readLine()) != null) {
                String[] d = linha.split(DELIMITER, -1);
                if (d.length >= MIN_COLS && DALUtil.parseInt(d[C_NUM_MEC]) == e.getNumMec()) {
                    pw.println(e.toCSV());
                    ok = true;
                } else {
                    pw.println(linha);
        ...
        } catch (IOException ex) { temp.delete(); DALUtil.log("EstudanteDAL", "atualizar", ex); return false; }

        return DALUtil.mover(ok, orig, temp);
    }
    ...
    public int gerarProximoNumMec(int anoAtual) {
        ...
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                String[] d = linha.split(DELIMITER, -1);
                if (d.length >= 1) {
                    int n = DALUtil.parseInt(d[0]);
                    if (n > max) max = n;
                }
            }
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "gerarProximoNumMec", e); }
        return max + 1;
    }

    /** Lista todos os estudantes (para ecrã de gestão do gestor). */
    public List<Estudante> listarTodos() {
        ...
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                String[] d = linha.split(DELIMITER, -1);
                if (d.length >= MIN_COLS) lista.add(mapear(d));
            }
        } catch (IOException e) { DALUtil.log("EstudanteDAL", "listarTodos", e); }
        return lista;
    }
    ...
    /** Retrocompatível: aceita 10 colunas (sem ativo) ou 11 (com ativo). */
    private Estudante mapear(String[] d) {
        boolean ativo = d.length >= 11 ? Boolean.parseBoolean(d[10].trim()) : true;
        return new Estudante(
                DALUtil.parseInt(d[C_NUM_MEC]),
                d[C_EMAIL].trim(),
                d[C_NOME].trim(),
                d[C_NIF].trim(),
                d[C_MORADA].trim(),
                d[C_DATA_NASC].trim(),
                DALUtil.parseInt(d[C_ANO_INS]),
                d[C_CURSO].trim(),
                DALUtil.parseDouble(d[C_SALDO]),
                DALUtil.parseInt(d[C_ANO_CURR]),
                ativo
        );
    }
    ...
    private BufferedReader abrir() {
        ...
        catch (IOException e) { DALUtil.log("EstudanteDAL", "abrir", e); return null; }
    }
