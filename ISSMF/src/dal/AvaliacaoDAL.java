...
public class AvaliacaoDAL {
    ...
    public boolean registarAvaliacao(int numAluno, String siglaUc, int anoLetivo,
        ...
        try {
            ...
            System.err.println("[AvaliacaoDAL] Erro ao registar: " + e.getMessage());
            return false;
        }
    }
    ...
    public List<Avaliacao> listarPorAluno(int numMec, int anoLetivo) {
        ...
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                if (d.length >= 6
                        && parseInt(d[0]) == numMec
                        && parseInt(d[2]) == anoLetivo) {
                    lista.add(mapear(d));
                }
            }
        } catch (IOException e) { log("listarPorAluno", e); }
        return lista;
    }
    ...
    public List<Avaliacao> listarPorSiglasUC(List<String> siglas) {
        ...
        } catch (IOException e) { log("listarPorSiglasUC", e); }
        return lista;
    }
        ...
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                if (d.length >= 6 && ucsDoDocente.contains(d[1].toUpperCase())) {
                    double nNormal = parseDouble(d[3]);
                    sb.append(String.format("  Aluno Nº %-10s | UC: %-8s | Normal: %-6s | Recurso: %-6s | Especial: %-6s%n",
                            d[0], d[1],
                            nNormal == -1 ? "Falta" : d[3],
                            parseDouble(d[4]) == -1 ? "Falta" : d[4],
                            parseDouble(d[5]) == -1 ? "Falta" : d[5]));
    ...
    private boolean existeAvaliacao(int numAluno, String siglaUc, int anoLetivo) {
        try (BufferedReader br = abrir()) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                String[] d = linha.split(DELIMITER, -1);
                if (d.length >= 3
                        && parseInt(d[0]) == numAluno
                        && d[1].equalsIgnoreCase(siglaUc)
                        && parseInt(d[2]) == anoLetivo) {
                    return true;
                }
            }
        } catch (IOException e) { log("existeAvaliacao", e); }
        return false;
    }
    ...
    private boolean atualizarAvaliacao(int numAluno, String siglaUc, int anoLetivo,
        ...
        if (encontrado) {
            try { Files.move(temp.toPath(), orig.toPath(),
                    java.nio.file.StandardCopyOption.REPLACE_EXISTING); return true; }
    ...
    private Avaliacao mapear(String[] d) {
        return new Avaliacao(
                parseInt(d[0]), d[1].trim(), parseInt(d[2]),
                parseDouble(d[3]), parseDouble(d[4]), parseDouble(d[5]));
    }
    ...
    private double parseDouble(String s) {
        try { return Double.parseDouble(s.trim()); } catch (NumberFormatException e) { return -1; }
    }

    private void log(String m, Exception e) {
        System.err.println("[AvaliacaoDAL." + m + "] " + e.getMessage());
    }
}
